...
select last_name, count(last_name) as 'How many actors have that last name' 
...
right join payment using (staff_id)
...
select title, count(*) as 'Times rented' from rental
...
select store_id, sum(amount) as 'dollars made ($)' from payment
...
select name as "Genre", sum(amount) as "Gross revenue" from payment 
